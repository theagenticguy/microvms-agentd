...
Three initiator families drive everything here: the `microvms-core` library's public lifecycle methods, the `agentd` HTTP routes the platform and clients call, and the `microvms-cli` command handlers. The eight processes below are the load-bearing ones; the rest are in `## Minor flows`.
...
3. The wire body is assembled with the constants injected rather than accepted: the one architecture value, the one OS capability derived from a boolean, and a `clientToken` minted from a label — `microvms-core/src/control/image.rs:165`.
...
7. `probe_stalled_build` reads the build list through `build_states`; an unreadable list or an empty one returns `Ok` rather than guessing, and only an all-`PENDING` list raises `ErrorKind::BuildWedged` naming the `clientToken` replay signature — `microvms-core/src/control/image.rs:291`.
...
1. `Sandbox::run` refuses a second launch on the same sandbox — STATE-3's local half, checked before any call because a retry loop around a timed-out launch is the plausible mistake — `microvms-core/src/sandbox.rs:525`.
...
5. On acceptance the lifecycle moves to `Pending`, `image_exists` is set, and `suspended_window` is recorded from *this* request — the only place the value is knowable, since `GetMicrovm` does not return it — `microvms-core/src/sandbox.rs:568`.
...
7. RUNNING is what sets `token_installed` and increments `bootstrap_count` — the platform reporting the run hook succeeded, not the launch call — `microvms-core/src/sandbox.rs:591`.
...
1. `Sandbox::suspend` resolves the VM id through `require_microvm`, then refuses any lifecycle but RUNNING with **zero** control-plane calls — the observable the test asserts on — `microvms-core/src/sandbox.rs:628`.
...
4. `wait_for_state` is called with `SUSPEND_WANTED`, so TERMINATED is a wanted outcome rather than a failure raised out of the middle of a teardown; anything else is an `ErrorKind::Platform` — `microvms-core/src/sandbox.rs:646`.
5. `Sandbox::resume` refuses a terminated VM first (STATE-11), then any lifecycle but SUSPENDED (STATE-7) — `microvms-core/src/sandbox.rs:708`.
6. `require_open_suspended_window` compares elapsed against the recorded window and refuses locally past it; with no window recorded — the attach path — it returns `Ok` rather than guessing a default — `microvms-core/src/sandbox.rs:768`.
7. `ResumeMicrovm` goes out and `wait_for_state` waits for RUNNING with `DEAD_STATES` as `fail_on`, not the terminal set, because SUSPENDED is the state the call was made from — `microvms-core/src/sandbox.rs:725`.
8. `Session::rebind` takes the endpoint the service just reported and invalidates the cached proxy token (STATE-8), and `suspended_at` is cleared so the next cycle measures its own window — `microvms-core/src/sandbox.rs:748`.
...
6. The image is deleted second, through `delete_image`'s retry loop — an image in `CREATING` refuses deletion and a VM still terminating holds a reference — `microvms-core/src/sandbox.rs:854`.
...
8. The build log group is handled **last**, and named rather than deleted: CloudWatch is absent from the dependency set, so it lands in `undeleted` with a failure line explaining why — `microvms-core/src/sandbox.rs:877`.
...
1. `routes::app` builds the router by walking `surface_docs()`, so a route cannot be served unless it is documented and a documented route with no handler panics at startup — `agentd/src/routes.rs:51`.
...
3. `run_hook` rejects a body that is not JSON with 400 — never 404, which a client would read as a missing artifact — `agentd/src/routes.rs:176`.
...
5. An empty `agent_token` is 400, and neither the token nor the payload carrying it is ever logged — `agentd/src/routes.rs:195`.
...
7. The three outcomes map to statuses: `Installed` and `AlreadyIdentical` are both 200 (the platform may retry its own hook, and answering 409 would fail a launch that is fine), `Conflict` is 409 — `agentd/src/routes.rs:200`.
...
1. `start` rejects a malformed body and an empty `exec_id` with 400, then validates `timeout_sec` — rejecting non-finite and non-positive values before anything is spawned, because doing it in the waiter left a running child with nobody to reap it — `agentd/src/exec.rs:353`.
2. `build_command` assembles the child: `sh -c` with one joined script when `shell` is set, `env_clear()` so the agent token cannot reach the child, an inherited cwd when none was given, `process_group(0)`, and `/dev/null` on stdin unless it was requested — `agentd/src/exec.rs:960`.
...
5. The pipes are taken out of the `Child` (which is also why the daemon must drop its own stdin copy on EOF), a broadcast channel is created, and `Shared` plus the registry entry are built — `agentd/src/exec.rs:1032`.
6. A detached task runs `super_wait`, which drains both pipes concurrently with `child.wait()` — a child filling a 64 KiB pipe buffer blocks forever if nobody reads — and escalates the process group on the timeout branch — `agentd/src/exec.rs:1145`.
...
1. `Transport::request` calls `self.headers(request_token(&request))`, so the mint sits inside the path every request takes — a token minted once at construction expires mid-run and the rejection is indistinguishable from a dead daemon — `microvms-core/src/session/mod.rs:114`.
2. The private `TOKEN_INTENT` marker header is stripped before the request leaves, and the caller's own headers are appended rather than replacing the vec — replacing it once stripped the content type and produced a daemon 400 while every fake-backed test stayed green — `microvms-core/src/session/mod.rs:115`.
...
6. `auth_value()` is checked *before* the token is cached, so an `authToken` map missing the auth key is a mint failure rather than a cached value that fails every request until the window rolls over — `microvms-core/src/session/proxy.rs:377`.
7. The token is cached with the clock reading, `mint_count` is incremented — the only externally visible evidence that minting happens in the request path — and `headers_from` adds the port header unless the token already carried one — `microvms-core/src/session/proxy.rs:380`.
...
2. Every precondition is checked before anything is created: the required role ARNs, and the daemon binary's existence when this invocation is the one building — so a missing role is not discovered after a 45-minute build — `microvms-cli/src/commands/lifecycle.rs:146`.
...
- Exec poll — entry at `agentd/src/exec.rs:407`. Strictly read-only against the registry; an acked entry reports no output so the payload cannot contradict the phase.
...
- Exec stdin write — entry at `agentd/src/exec.rs:682`. Four distinct statuses (409 not-requested, 410 closed, 413 too large, 408 write timeout) and an EOF that drops the daemon's own pipe handle.
...
- Tar upload and extraction — entry at `agentd/src/fs.rs:837`. The one confined write path: spools the body under a disk pacer, then extracts with deferred directory modes, refused device nodes, and lexically resolved link targets.
...
- CLI attached suspend — entry at `microvms-cli/src/commands/lifecycle.rs:730`. Reads the state with `GetMicrovm` first and refuses locally from anything but RUNNING, which is STATE-5's local half on a path that did not launch.
- CLI attached resume — entry at `microvms-cli/src/commands/lifecycle.rs:795`. Deliberately does not check the suspended window, since a process that did not send the launch cannot know it; relies on `fail_on: DEAD_STATES` instead.
...
- CLI stdin — entry at `microvms-cli/src/commands/attached.rs:612`. Refuses a no-op write locally and chunks under the daemon's per-write cap with EOF riding the final chunk.
- CLI cp — entry at `microvms-cli/src/commands/attached.rs:779`. Resolves direction from the `vm:` prefix before opening anything, and never inspects an archive so the daemon's extractor stays the only one under test.
...
- CLI logs — entry at `microvms-cli/src/commands/local.rs:146`. Derives and names the build log group, then deliberately fails with `lines: null` rather than an empty array, which would read as "the group had no events".
...
- CLI cost — entry at `microvms-cli/src/commands/cost.rs:27`. Refuses negative durations up front, then renders a report from pinned rates with unpriced line items kept distinct from zero.
