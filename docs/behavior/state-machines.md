...
This document describes three state machines. Each is declared once as a Rust enum and
mirrored elsewhere by convention rather than by a cargo dependency, so the mirrors are
listed beside each diagram.

## ExecPhase

`ExecPhase` records where one exec sits in its lifecycle. It has three states, and output
is held until the caller acks, which is what makes a retried poll safe.
...
- `kill` signals the process group and leaves the phase unchanged; the phase moves only when
  the child actually exits — `agentd/src/exec.rs:886-921`. It is therefore not a transition
  of this machine.
...
- `agentd/src/exec.rs:1108-1116` — `phase_of(acked, finished)`. The daemon derives the phase
  from `acked_at.is_some()` and `result.is_some()` instead of storing a phase field.
...
`model/src/lib.rs:501-506` confirm that the checker reached `Acked` and a retried start.
...
`Lifecycle` records one MicroVM's whole life. It is held as a private field, and only the
five methods of `Sandbox` mutate it. The six states are the symspec's `vm_state`, verbatim.
...
- `terminate` sets `Terminating` before the call, so a terminate whose call fails still marks
  ...
  stays at `Terminating` — `microvms-core/src/sandbox.rs:843-848`.

The following guards all refuse before any control-plane call is made:
...
Model checking found a bug behind the third invariant that code reading had missed. A resume
issued legally from `Suspended`, followed by a terminate, followed by the resume's completion
arriving late, put a `was_terminated` VM back in `Running`. The fix makes a completion apply
only while a resume is still in flight *and* the state is still `Suspended`, so the terminate
wins — `model/src/client.rs:164-173`, `model/src/client.rs:451`.
...
`StreamState` is the SSE output stream's cursor machine. It tracks where an attach is, how
many times it has dropped, and the byte offset a resume would ask for. One step function
serves both the `Stream` and the callback driver, so there is one implementation of the
reconnect rule.
...
- a fatal failure goes to `Done` with the error, because reconnecting can never succeed. The
  main example is a 404 on a collected entry.
...
- `Gap` advances the cursor to `to` unconditionally, so a reconnect does not ask for the
  evicted range again and receive the same gap forever. It then stays `Attached`, or goes to
  `Done` with an `OutputGap` error when `options.error_on_gap` is set —
  `microvms-core/src/session/exec.rs:458-480`.
- `Exit` goes to `Done`. A finished command always delivers this event, so a body that ends
  without it is a cut connection — `microvms-core/src/session/exec.rs:483-485`,
...
`StreamState::cursor()` returns `None` for `Done`. `Done` is reached from several places, so
any number returned there could shadow the last real cursor the caller already holds —
`microvms-core/src/session/exec.rs:678-685`.

`for_each_event` reports which `Done` path was taken as `EndReason`
(`microvms-core/src/session/exec.rs:142-153`). `EndReason` is a return classification, not a
state the machine occupies. It is `Exited` when the terminal `Exit` event was delivered,
`Stopped` when the callback answered `ControlFlow::Break`, and `Cut` when the body ended with
no `Exit` event and reconnecting was refused — `microvms-core/src/session/exec.rs:340-372`.
The returned cursor is read off the machine rather than recomputed from the events. A caller
that resumes therefore holds one cursor, not a second one that could disagree exactly when a
gap arrived — `microvms-core/src/session/exec.rs:350-355`.
