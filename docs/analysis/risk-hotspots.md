...
**Findings.** 1 error-class, 7 warn-class — the highest distinct-trap count of any file. The error is credential leakage: both `RunHookPayload`'s `Debug` (`microvms-core/src/control/microvm.rs:78`) and `ProxyToken`'s (`microvms-core/src/control/microvm.rs:222`) were derived and printed live secrets, while three sibling types already guarded the same invariant by hand; each now redacts, proven by restoring the derive and watching the guard go red. The most instructive warn is TRAP-5 itself, because the service's own documentation reproduces the bug: the model's doc string for `runHookPayload` reads "Maximum: 16,384 bytes" while its shape `RunMicrovmRequestRunHookPayloadString` declares `max: 4096`, so a reader who checks the docs rather than the shape lands 4x wrong in the dangerous direction (`microvms-core/src/control/microvm.rs:10-17`, corroborated at `docs/PLATFORM.md:56-76`). The live-coverage gap is the payload ceiling and the terminal-state fast-fail: 25 inline tests cover them, but no live check pushes a 4097-byte payload or kills a VM before `RUNNING`, since both would cost a launch to observe.
...
**Findings.** 1 error-class, 0 warn-class. The error is a denial-of-service shape the offline tiers could not see: the parser rescanned its whole buffer from byte zero on every chunk with no ceiling, so 6.5 MB of unterminated stream cost **31 seconds of CPU** inside the async task. The fix is a resuming cursor that backs up `MAX_TERMINATOR_LEN - 1` so a straddling terminator is still found (`microvms-core/src/session/sse.rs:34`), plus a 4 MiB pending ceiling that errors as `Protocol` — fatal to the stream loop rather than a reconnect into the same wedge (`microvms-core/src/session/sse.rs:68`, `microvms-core/src/session/sse.rs:115-125`). The threat model is written into the constant's doc: the thing on the other end is a *proxy*, and a proxy answering a stream request with an HTML error page produces exactly that shape, with no frame boundary ever coming (`microvms-core/src/session/sse.rs:39-46`). The regression test names the measured shape — 800 unterminated 8 KiB reads (`microvms-core/src/session/sse.rs:514`). The live-coverage gap: `drive_streaming` exercises well-formed streaming (`conformance/run_rs.py:1282`), but no live check feeds the parser a hostile non-SSE body, so the ceiling is proven only by the 13 inline tests.
...
**Findings.** 0 error-class, 10 warn-class — tied for the highest distinct-trap count, and the reason the scoring weight had to be corrected. These 10 are references rather than resident hazards; the file's real risk is that it is the load-bearing argument for why the other guards exist. Two entries carry that weight. TRAP-11 is closed **by absence**: `CreateMicrovmShellAuthToken` is in the service model and deliberately unimplemented, so the test for it counts the calls a full lifecycle made rather than asserting a refusal (`microvms-core/src/control/mod.rs:26-37`). And the file states why every guard downstream is load-bearing rather than belt-and-braces: botocore's `VALIDATED_METADATA_ATTRS` is `{required, min, document, union}`, so `max`, `pattern`, and `enum` violations go to the wire and "the SDK validates the model already" was never true (`microvms-core/src/control/mod.rs:39-44`). The live-coverage gap is the whole S1-by-absence class — a trap closed by a method not existing cannot be probed live at all, which is what makes the 11 inline tests and the `spec:core` requirement tier the only oracle.

### microvms-core/src/cost.rs

**What's there.** What a run cost and what a plan will cost, with every figure labeled by provenance. Two rules hold throughout: seconds are measured and dollars are estimated, so there is no type for an actual dollar amount, only `EstimatedUsd` (`microvms-core/src/cost.rs:15-21`); and unknown is not zero, so `Amount::Unpriced` carries a reason as a distinct variant a consumer must handle (`microvms-core/src/cost.rs:23-28`).
