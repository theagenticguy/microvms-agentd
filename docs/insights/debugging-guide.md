...
| `microvm logs <image-name>` | A **failure** envelope rather than an empty success, deliberately: an empty list reads as "there are no logs" when the real answer is "this client cannot read them" | `data.logGroup` plus the `aws logs tail` invocation in `suggestions`. CloudWatch is not in the transport's dependency set | `microvms-cli/src/commands/local.rs:148-174` |
